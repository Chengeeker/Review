...
class ReviewDocumentsProvider : DocumentsProvider() {
    ...
    override fun createDocument(
        ...
        val newFile = resolveDocument(File(parent, displayName).path)
    ...
    override fun deleteDocument(documentId: String) {
        ...
        if (file == rootDirectory()) {
            throw SecurityException("The application root cannot be deleted")
        }
    ...
    override fun renameDocument(documentId: String, displayName: String): String {
        val file = resolveDocument(documentId)
        if (file == rootDirectory()) {
            throw SecurityException("The application root cannot be renamed")
        }
        if (!file.exists()) {
            throw FileNotFoundException("File not found: $documentId")
        }
        val parent = file.parentFile ?: throw FileNotFoundException("Cannot rename root")
        val target = resolveDocument(File(parent, displayName).path)
    ...
    private fun rootDirectory(): File {
        val ctx = context ?: throw SecurityException("Provider is not attached")
        ...
        return (targetDir ?: throw SecurityException("Application data directory is unavailable")).canonicalFile
    }
    ...
    private fun resolveDocument(documentId: String): File {
        ...
        if (file != root && !file.path.startsWith(root.path + File.separator)) {
            throw SecurityException("Document is outside the application root")
        }
        return file
    }
