...
void main() {
  group('Weibo Detail Interaction Tests', () {
    ...
    test(
        'WeiboCommentModel correctly identifies AI generation (千问, AI bots, etc.)',
        () {
    ...
    test('nested reply keeps avatar and blogger identity fields', () {
      final comment = WeiboCommentModel.fromJson({
        'id': 'comment-1',
        'text_raw': '主评论',
        'user': {
        ...
        'comments': [
          {
            ...
            'user': {
              'id': 'blogger-1',
              'screen_name': '博主',
              'avatar_large': 'https://tvax1.sinaimg.cn/reply.jpg',
              'verified': true,
              'verified_type': 0,
            },
          },
      ...
      final reply = comment.subComments.single;
      expect(reply.user.id, equals('blogger-1'));
      expect(reply.user.screenName, equals('博主'));
      expect(reply.user.avatar, contains('reply.jpg'));
      expect(reply.user.verified, isTrue);
      expect(reply.user.verifiedType, equals(0));
    });
  });
}
