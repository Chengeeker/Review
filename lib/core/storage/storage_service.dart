...
class StorageService {
  ...
  static const String keyScreenRefreshRateMode =
      'key_screen_refresh_rate_mode'; // 0~8
  static const String keyCurrentPresetAppIconId =
      'key_current_preset_app_icon_id'; // 'default' | 'alias1' | 'alias2' | 'alias3'

  // Card Display & Date Settings Keys
  static const String keyTimeDisplayMode =
      'time_display_mode'; // 'relative' | 'absolute'
  ...
  static const String keyCardBackgroundLayout =
      'card_background_layout'; // 'card_rounded' | 'flat_tile'
  static const String keyWeiboFontSize =
      'weibo_font_size'; // double, default 16.0
  static const String keyWeiboFontLineHeight =
      'weibo_font_line_height'; // double, default 1.3
  static const String keyLinkColorFollowTheme =
      'link_color_follow_theme'; // bool, default true
  static const String keyShowRemarkAndName =
      'show_remark_and_name'; // bool, default false
  static const String keyShowBackgroundImage =
      'show_background_image'; // bool, default false
  static const String keyShowUserActivityIcon =
      'show_user_activity_icon'; // bool, default true
  static const String keyLargeImageMode =
      'large_image_mode'; // bool, default true
  static const String keyRoundedImageCorners =
      'rounded_image_corners'; // bool, default true
  static const String keyShowMenuAtBottom =
      'show_menu_at_bottom'; // bool, default true
  static const String keyShowIpLocationMode =
      'show_ip_location_mode'; // 'all' | 'detail_only' | 'none'
  static const String keyShowProfileLikedTweets =
      'show_profile_liked_tweets'; // bool, default false
  ...
  Future<bool> setSubCookie(String value) =>
      _prefs.setString(keySubCookie, value);

  String? getSubpCookie() => _prefs.getString(keySubpCookie);
  Future<bool> setSubpCookie(String value) =>
      _prefs.setString(keySubpCookie, value);
  ...
  Future<bool> setUseFloatingNavBar(bool val) =>
      _prefs.setBool(keyUseFloatingNavBar, val);

  int getImageSavePathType() => getInt(keyImageSavePathType, defaultValue: 0);
  Future<bool> setImageSavePathType(int value) =>
      setInt(keyImageSavePathType, value);

  int getVideoSavePathType() => getInt(keyVideoSavePathType, defaultValue: 0);
  Future<bool> setVideoSavePathType(int value) =>
      setInt(keyVideoSavePathType, value);

  String? getSavedDefaultGroupsJson() =>
      _prefs.getString(keySavedDefaultGroups);
  Future<bool> setSavedDefaultGroupsJson(String json) =>
      _prefs.setString(keySavedDefaultGroups, json);

  String? getSavedPersonalGroupsJson() =>
      _prefs.getString(keySavedPersonalGroups);
  Future<bool> setSavedPersonalGroupsJson(String json) =>
      _prefs.setString(keySavedPersonalGroups, json);

  String? getSavedHotGroupsJson() => _prefs.getString(keySavedHotGroups);
  Future<bool> setSavedHotGroupsJson(String json) =>
      _prefs.setString(keySavedHotGroups, json);

  bool getEnableHaptics() => _prefs.getBool(keyEnableHaptics) ?? true;
  Future<bool> setEnableHaptics(bool val) =>
      _prefs.setBool(keyEnableHaptics, val);

  bool getHotSearchVerticalLayout() =>
      getBool(keyHotSearchVerticalLayout, defaultValue: false);
  Future<bool> setHotSearchVerticalLayout(bool val) =>
      setBool(keyHotSearchVerticalLayout, val);

  bool getUseCustomFontWeight() =>
      getBool(keyUseCustomFontWeight, defaultValue: false);
  Future<bool> setUseCustomFontWeight(bool val) =>
      setBool(keyUseCustomFontWeight, val);

  int getCustomFontWeightDelta() =>
      getInt(keyCustomFontWeightDelta, defaultValue: 0);
  Future<bool> setCustomFontWeightDelta(int val) =>
      setInt(keyCustomFontWeightDelta, val);

  int getScreenRefreshRateMode() =>
      getInt(keyScreenRefreshRateMode, defaultValue: 0);
  Future<bool> setScreenRefreshRateMode(int val) =>
      setInt(keyScreenRefreshRateMode, val);

  // Card Display getters & setters
  String getTimeDisplayMode() =>
      _prefs.getString(keyTimeDisplayMode) ?? 'relative';
  Future<bool> setTimeDisplayMode(String mode) =>
      _prefs.setString(keyTimeDisplayMode, mode);
  ...
  Future<bool> setShowTimezone(bool val) =>
      _prefs.setBool(keyShowTimezone, val);
  ...
  Future<bool> setAppLanguage(String lang) =>
      _prefs.setString(keyAppLanguage, lang);

  // Weibo Style Getters & Setters
  String getWeiboSuffix() => _prefs.getString(keyWeiboSuffix) ?? '来自微博国际版';
  Future<bool> setWeiboSuffix(String suffix) =>
      _prefs.setString(keyWeiboSuffix, suffix);

  String getCardBackgroundLayout() =>
      _prefs.getString(keyCardBackgroundLayout) ?? 'card_rounded';
  Future<bool> setCardBackgroundLayout(String layout) =>
      _prefs.setString(keyCardBackgroundLayout, layout);

  double getWeiboFontSize() => _prefs.getDouble(keyWeiboFontSize) ?? 16.0;
  Future<bool> setWeiboFontSize(double size) =>
      _prefs.setDouble(keyWeiboFontSize, size);

  double getWeiboFontLineHeight() =>
      _prefs.getDouble(keyWeiboFontLineHeight) ?? 1.3;
  Future<bool> setWeiboFontLineHeight(double height) =>
      _prefs.setDouble(keyWeiboFontLineHeight, height);

  bool getLinkColorFollowTheme() =>
      _prefs.getBool(keyLinkColorFollowTheme) ?? true;
  Future<bool> setLinkColorFollowTheme(bool val) =>
      _prefs.setBool(keyLinkColorFollowTheme, val);

  bool getShowRemarkAndName() => _prefs.getBool(keyShowRemarkAndName) ?? false;
  Future<bool> setShowRemarkAndName(bool val) =>
      _prefs.setBool(keyShowRemarkAndName, val);

  bool getShowBackgroundImage() =>
      _prefs.getBool(keyShowBackgroundImage) ?? false;
  Future<bool> setShowBackgroundImage(bool val) =>
      _prefs.setBool(keyShowBackgroundImage, val);

  bool getShowUserActivityIcon() =>
      _prefs.getBool(keyShowUserActivityIcon) ?? true;
  Future<bool> setShowUserActivityIcon(bool val) =>
      _prefs.setBool(keyShowUserActivityIcon, val);

  bool getLargeImageMode() => _prefs.getBool(keyLargeImageMode) ?? true;
  Future<bool> setLargeImageMode(bool val) =>
      _prefs.setBool(keyLargeImageMode, val);

  bool getRoundedImageCorners() =>
      _prefs.getBool(keyRoundedImageCorners) ?? true;
  Future<bool> setRoundedImageCorners(bool val) =>
      _prefs.setBool(keyRoundedImageCorners, val);

  bool getShowMenuAtBottom() => _prefs.getBool(keyShowMenuAtBottom) ?? true;
  Future<bool> setShowMenuAtBottom(bool val) =>
      _prefs.setBool(keyShowMenuAtBottom, val);

  String getShowIpLocationMode() =>
      _prefs.getString(keyShowIpLocationMode) ?? 'all';
  Future<bool> setShowIpLocationMode(String mode) =>
      _prefs.setString(keyShowIpLocationMode, mode);

  bool getShowProfileLikedTweets() =>
      _prefs.getBool(keyShowProfileLikedTweets) ?? false;
  Future<bool> setShowProfileLikedTweets(bool val) =>
      _prefs.setBool(keyShowProfileLikedTweets, val);

  List<String> getSearchHistory() =>
      _prefs.getStringList(keySearchHistory) ?? [];
  ...
  static const String keyBrowsingHistoryStatuses =
      'wb_browsing_history_statuses';

  List<String> getBrowsingHistoryStatusJsons() =>
  ...
  Future<bool> clearBrowsingHistory() async =>
      _prefs.remove(keyBrowsingHistoryStatuses);

  List<String> getBlockedUserIds() =>
      _prefs.getStringList(keyBlockedUserIds) ?? [];

  Future<bool> addBlockedUser(String uid) async {
    final cleanUid = uid.trim();
    if (cleanUid.isEmpty) return false;
    final ids = List<String>.from(getBlockedUserIds());
    if (!ids.contains(cleanUid)) ids.add(cleanUid);
    return _prefs.setStringList(keyBlockedUserIds, ids);
  }
  ...
  Future<bool> setWebDavUsername(String v) =>
      _prefs.setString(keyWebDavUsername, v);

  String getWebDavPassword() => _prefs.getString(keyWebDavPassword) ?? '';
  Future<bool> setWebDavPassword(String v) =>
      _prefs.setString(keyWebDavPassword, v);

  String getWebDavDirectory() =>
      _prefs.getString(keyWebDavDirectory) ?? 'Review';
  Future<bool> setWebDavDirectory(String v) =>
      _prefs.setString(keyWebDavDirectory, v);

  String getCurrentPresetAppIconId() =>
      _prefs.getString(keyCurrentPresetAppIconId) ?? 'default';
  Future<bool> setCurrentPresetAppIconId(String v) =>
      _prefs.setString(keyCurrentPresetAppIconId, v);
