...
class WeiboTextParser {
  ...
  static String? _imageLabel(String tag) {
    final label = _attributeValue(tag, 'alt') ??
        _attributeValue(tag, 'title') ??
        _attributeValue(tag, 'data-emoji');
    if (label == null ||
        !label.startsWith('[') ||
        !label.endsWith(']') ||
        label.length < 3) {
  ...
  static Map<String, String> _extractEmojiUrls(String? html) {
    if (html == null || html.isEmpty) return const {};
    final result = <String, String>{};
    for (final match in _imageTagRegex.allMatches(html)) {
      final tag = match.group(0)!;
      final label = _imageLabel(tag);
      if (label == null) continue;
      final url = _normalizeImageUrl(
        _attributeValue(tag, 'data-original') ??
            _attributeValue(tag, 'data-src') ??
            _attributeValue(tag, 'src') ??
            _attributeValue(tag, 'url'),
      );
      if (url != null) result[label] = url;
    }
    return result;
  }
  ...
  static List<InlineSpan> parse({
    ...
    final inlineEmojiUrls = <String, String>{
      ..._extractEmojiUrls(rawText),
      ..._extractEmojiUrls(htmlText),
    };
    ...
    for (final match in _weiboRegex.allMatches(cleanText)) {
      ...
      if (matchedText.startsWith('@')) {
        ...
        final cleanTopic =
            topic.replaceAll('[超话]', '').replaceAll('超话', '').trim();
        ...
        if (urlStruct != null) {
          for (final u in urlStruct) {
            ...
            if (sUrl == matchedText ||
                uTitle.contains(cleanTopic) ||
                oriUrl.contains(cleanTopic)) {
              ...
              ..onTap = () {
                if (isChaohua) {
                  ...
                  if (containerId.isEmpty &&
                      chaohuaStruct?['ori_url'] != null) {
                    final m = RegExp(r'containerid=([^&]+)')
                        .firstMatch(chaohuaStruct!['ori_url']!.toString());
        ...
        final isChaohuaLink = pageId.startsWith('100808') ||
            oriUrl.contains('containerid=100808');
        ...
        // The status HTML contains the exact asset selected by Weibo. Prefer
        // it over the static compatibility map so newly added emotions (for
        // example [勾引]) render without waiting for a client release.
        final emojiUrl = inlineEmojiUrls[matchedText] ??
            WeiboEmojis.getEmojiUrl(matchedText);
