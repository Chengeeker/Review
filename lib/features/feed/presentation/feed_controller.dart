...
class FeedController extends StateNotifier<FeedState> {
  ...
  void _loadInitialGroupPreferences() {
    ...
    List<Map<String, String>> persGroups = [];
    ...
    if (persJson != null) {
      try {
        ...
        final parsed = list
            ...
            .where((group) => !_isSyntheticGroup(group))
            .toList();
  ...
  Future<void> updateGroupPreferences({
    ...
    final validPersonalGroups =
        personalGroups.where((group) => !_isSyntheticGroup(group)).toList();
    await _storage.setSavedPersonalGroupsJson(jsonEncode(validPersonalGroups));
    await _storage.setSavedHotGroupsJson(jsonEncode(hotGroups));

    state = state.copyWith(
      customDefaultGroups: defaultGroups,
      customPersonalGroups: validPersonalGroups,
      customHotGroups: hotGroups,
    );
  }

  bool _isSyntheticGroup(Map<String, String> group) {
    return group['id']?.startsWith('custom_') ?? false;
  }
  ...
  Future<void> initAndLoad() async {
    try {
      ...
      // 1. 实时抓取微博云端个人分组。接口明确返回空时也要清掉过期缓存，
      // 否则旧账号或另一账号的分组会继续出现在当前账号中。
      if (groupsResult.loaded) {
    ...
    // 首次安装/更新后，微博桌面接口偶尔会在会话刚恢复时返回空页。
    // 只重试一次，避免把暂时性空结果误显示成“暂无微博内容”。
    await refreshFeed(retryOnEmpty: true);
  }
  ...
  Future<void> setCategory(String category, [String? title]) async {
    ...
    if (title == null) {
      if (category == 'friends' || category == 'all_follow') {
        ...
        final match =
            state.userGroups.where((g) => g.gid == category).firstOrNull;
        if (match != null) {
          resolvedTitle = match.title;
        } else {
          final customMatch = state.customPersonalGroups
              .where((g) => g['id'] == category)
              .firstOrNull;
          if (customMatch != null) {
            resolvedTitle = customMatch['name'] ?? resolvedTitle;
          }
        }
      }
    }
  ...
  Future<void> refreshFeed({bool retryOnEmpty = false}) async {
    ...
    // 状态本身已被清空，因此仍会正常显示真正的空结果。
    final preserveExisting =
        result.statuses.isEmpty && state.statuses.isNotEmpty;
    final nextStatuses = preserveExisting ? state.statuses : visibleStatuses;
    final nextMaxId = preserveExisting
        ? state.maxId
        : result.maxId != '0'
            ? result.maxId
            : (visibleStatuses.isNotEmpty ? visibleStatuses.last.id : '0');
    ...
    state = state.copyWith(
      ...
      // Filtering blocked users must not make the remote page look exhausted.
      hasMore: preserveExisting ? state.hasMore : result.statuses.isNotEmpty,
      isLoading: false,
      errorMessage: preserveExisting
          ? null
          : result.error ??
  ...
  Future<bool> loadMore() async {
    ...
    try {
      ...
      final uniqueNew =
          visibleStatuses.where((s) => !existingIds.contains(s.id)).toList();
  ...
  List<WeiboStatusModel> _withoutBlockedUsers(List<WeiboStatusModel> statuses) {
    final blocked = _storage.getBlockedUserIds().toSet();
    if (blocked.isEmpty) return statuses;
    return statuses
        .where((status) => !blocked.contains(status.user.id))
        .toList();
  }

  Future<void> blockUser(String uid) async {
    if (await _storage.addBlockedUser(uid)) {
      final blocked = _storage.getBlockedUserIds().toSet();
      state = state.copyWith(
  ...
  void syncFavoriteLocally(String statusId, bool favorited) {
    final updated = state.statuses.map((s) {
      return s.id == statusId ? s.copyWith(favorited: favorited) : s;
    }).toList();
    state = state.copyWith(statuses: updated);
  }

  Future<bool> toggleLikeLocally(String statusId,
      {bool? currentlyLiked}) async {
    ...
    final updated = state.statuses.map((s) {
      if (s.id == statusId) {
        ...
        return s.copyWith(
          liked: nextLiked,
          attitudesCount: nextLiked
              ? s.attitudesCount + 1
              : (s.attitudesCount > 0 ? s.attitudesCount - 1 : 0),
        );
      }
      return s;
    }).toList();
    ...
    try {
      final success =
          await _repository.toggleLike(statusId, currentlyLiked: wasLiked);
      if (!success) {
...
final timelineScrollProvider =
    StateNotifierProvider<TimelineScrollNotifier, double>((ref) {
