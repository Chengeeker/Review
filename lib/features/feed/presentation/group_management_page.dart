...
class _GroupManagementPageState extends ConsumerState<GroupManagementPage>
  ...
  void initState() {
    ...
    _myPersonalGroups =
        List<Map<String, String>>.from(feedState.customPersonalGroups);
    _myDefaultGroups =
        List<Map<String, String>>.from(feedState.customDefaultGroups);
    if (_myDefaultGroups.isEmpty) {
      _myDefaultGroups =
          List<Map<String, String>>.from(FeedController.defaultInitialGroups);
    }
    _myHotGroups = List<Map<String, String>>.from(feedState.customHotGroups);

    final myDefIds = _myDefaultGroups.map((e) => e['id']).toSet();
    _recommendedDefaultGroups = allDefaultCandidateGroups
        .where((e) => !myDefIds.contains(e['id']))
        .toList();

    final myHotIds = _myHotGroups.map((e) => e['id']).toSet();
    _recommendedHotGroups = allHotCandidateGroups
        .where((e) => !myHotIds.contains(e['id']))
        .toList();
  }
  ...
  void _showCreateGroupDialog() {
    showAppDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('暂不支持新建云端分组'),
        content: const Text('当前只能使用微博账号已有的个人分组。本地伪造分组无法读取微博内容，因此已停止创建。'),
        actions: [
          FilledButton(
                ...
                onPressed: () {
                  ...
                  showDialog(
                    ...
                    builder: (ctx) => AlertDialog(
                      ...
                      content: TextField(
                          controller: editController, autofocus: true),
                      actions: [
                        TextButton(
