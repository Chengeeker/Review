...
class _SearchViewState extends ConsumerState<SearchView> {
  ...
  bool _isHotSearchVertical = false;
  ...
  void initState() {
    super.initState();
    _loadHistory();
    _isHotSearchVertical =
        ref.read(storageServiceProvider).getHotSearchVerticalLayout();
    _fetchHotSearch();
    ...
    return ListView(
      ...
      children: [
        ...
        if (users.isNotEmpty) ...[
          ...
          ...users
              .map((user) => _buildUserSuggestCard(context, user, colorScheme)),
        ...
        if (suggestions.isNotEmpty) ...[
          ...
          ...suggestions.map((word) =>
              _buildSuggestWordRow(context, word, query, colorScheme)),
  ...
  Widget _buildHotSearchSection(BuildContext context, ColorScheme colorScheme) {
    final itemLimit = _isHotSearchVertical ? 10 : 9;
    final hotItems = _hotList.take(itemLimit).toList();
      ...
      children: [
            ...
            Row(
              children: [
                const Icon(Icons.local_fire_department_rounded,
                    size: 19, color: Color(0xFFFF5722)),
                const SizedBox(width: 6),
                Text(
                  '热门搜索',
                  style: TextStyle(
            ...
            Row(
              ...
              children: [
                IconButton(
                  ...
                  tooltip: _isHotSearchVertical ? '切换为横向显示' : '切换为竖向显示',
                  visualDensity: VisualDensity.compact,
                  onPressed: () {
                    HapticFeedbackUtil.light();
                    final next = !_isHotSearchVertical;
                    setState(() => _isHotSearchVertical = next);
                    ref
                        .read(storageServiceProvider)
                        .setHotSearchVerticalLayout(next);
                  },
                ),
                IconButton(
                  icon: Icon(
        ...
        if (_isLoadingHot)
          ...
          Container(
            ...
            child: _buildHotSearchItems(context, hotItems, colorScheme),
          ),
      ],
    );
  }

  Widget _buildHotSearchItems(
    BuildContext context,
      ...
      return ListView.builder(
        ...
        itemBuilder: (context, index) {
          if (index < items.length) {
            return _buildCompactHotItem(
                context, items[index], index, colorScheme);
          }
  ...
  Widget _buildCompactHotItem(
    ...
    final rankColor = switch (index) {
      0 => const Color(0xFFFF2442),
      1 => const Color(0xFFFF7700),
      2 => const Color(0xFFFFB300),
      _ => colorScheme.onSurfaceVariant.withValues(alpha: 0.6),
    };
    ...
    return InkWell(
      ...
      child: Padding(
        padding: EdgeInsets.symmetric(
          horizontal: 4,
          vertical: _isHotSearchVertical ? 8 : 4,
        ),
        child: Row(
          children: [
            // 序号
  ...
  /// 第 10 个位置：“更多热搜”按钮
  Widget _buildMoreHotSearchButton(
      BuildContext context, ColorScheme colorScheme) {
    return InkWell(
      borderRadius: BorderRadius.circular(8),
      onTap: () {
